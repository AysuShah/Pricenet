...
public class JsoupItemNoutbuk {
    public static void main(String[] args) throws IOException {


        Document pageCount = Jsoup.connect("https://qiymeti.net/qiymetleri/notebook/").get();
        ...
        for (int i = 1; i <= Integer.parseInt(pageNumber); i++) {
            ...
            Document document = Jsoup.connect(" https://qiymeti.net/qiymetleri/notebook/page/" + i).get();

            Elements products = document.getElementsByClass("product");
            for (Element product : products) {
                Elements minprice3 = product.getElementsByClass("min-price");
                ...
                Document document2 = Jsoup.connect("https://qiymeti.net/notebook/" + attr).get();
