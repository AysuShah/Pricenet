...
public class JsoupItemCamera {
    public static void main(String[] args) throws IOException {


        Document pageCount = Jsoup.connect("https://qiymeti.net/qiymetleri/fotoaparat/").get();
        ...
        for (int i = 1; i <= Integer.parseInt(pageNumber); i++) {
            ...
            Document document = Jsoup.connect("https://qiymeti.net/qiymetleri/fotoaparat/page/" + i).get();

            Elements products = document.getElementsByClass("product");
            for (Element product : products) {
                Elements minprice2 = product.getElementsByClass("min-price");
